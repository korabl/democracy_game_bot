...
async def generate_world_news(world_data, world_metrics, model="gpt-4o", max_tokens=6800):
    try:
        ...
        Пожалуйста, создай дайджест новостей в стиле Times или Bloomberg, который описывает текущее состояние мира, включая важные события, изменения и тенденции, которые происходят в нем.
        ...
        Не используй для флорматирования текста **жирный** или _курсив_, так как это не поддерживается в данном формате. Используй капслок для заголовков 
        и кратких выделений.
...
async def generate_world_changes(world_data, user_initiation, model="gpt-4o", max_tokens=800):
    try:
        ...
        Контекстуальные ответы NPC: Пришли текстовое описание новой реальности исходя из инициативы пользователя от лица одного из граждан этой страны - 
        это может быть любой человек. Это даст игроку ощущение, что NPC живут в этом мире.
        Используй различные стилистические повествования (можешь говорить в стиле известных персонажей из медиакультуры), чтобы передать эмоции гражданина, можешь даже поделиться какими-то его личными историями, 
        главное эмоционально вовлечь пользователя и показать ему, что его инициативы влияют на настоящих людей. Опиши несколькими абзацами до 800 символов. 
        
        Оформи текст как, чтобы наладит эмоциональную связь с НПС, имитируй разную речь и повествование, подходящее НПС. Учитывай, что у НПС могут быть разные взгляды
        на происзодящее в стране.

        Пример:
        Имя Фамилия НПС
        профессия на заводе
        в свободное время любит читать комиксы 

        Текст о том, что произощло в стране за последний год. Как это отразилось на него. Что он об этом думает и чувствует.
